...
libpath_local <- c("/home/grionan1/dev/R_lib/session9_gsva/")
...
```{r renv}
if (!requireNamespace("renv", quietly = TRUE, lib.loc=libpath_local))
    install.packages("renv", lib=libpath_local)
if (!requireNamespace("glue", quietly = TRUE, lib.loc=libpath_local))
...
cm <- makeContrasts(
  PBOvsTRT = ARMplacebo - ARMtreatment,
  levels = design
)
...
allGeneSets <- topTable(fit, coef="PBOvsTRT", number=Inf)
